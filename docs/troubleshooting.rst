...
garbage collection (`LockBase.__del__`) and a module level `atexit`
hook that releases any lock still held at interpreter exit, in the
process that constructed it. None of those three run when the process
is killed outright -- ``SIGKILL``, a segfault, a lost VM -- so the file
stays on disk.
